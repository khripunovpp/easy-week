...
_EXTRACT_SYSTEM = (
    "Извлеки УСТОЙЧИВЫЕ пищевые предпочтения пользователя из его сообщения. "
    "dislikes — что не любит / просит избегать / аллергии / ограничения "
    "(напр. «чечевица», «свинина», «острое»). likes — что любит / просит чаще. "
    "Только короткие названия продуктов/блюд/ограничений на русском. "
    "Бери ТОЛЬКО явные устойчивые предпочтения, НЕ разовые пожелания к конкретному плану "
    "(«сегодня хочу», «на этой неделе») и НЕ количество блюд/порции. "
    "Если предпочтений нет — верни пустые списки."
)

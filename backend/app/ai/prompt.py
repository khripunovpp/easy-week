...
FREEZE_RULES = (
    ...
    "НИКОГДА не морозить в блюде (расслаиваются, крахмал даёт зернистость и воду) — добавлять "
    "ТОЛЬКО свежими, отдельно при подаче или при разогреве, и обязательно писать это в шагах и "
    "note: сливки, сметану, молоко, йогурт, любые сливочные соусы; мягкий и плавленый сыр; крахмал "
    "и мучные загустители (бешамель, заварные соусы, кисель — вводить/размешивать при разогреве, "
    "блюдо не морозить уже загущённым). Твёрдый сыр после разморозки крошится — только в "
    "готовку/запекание. "

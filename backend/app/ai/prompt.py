NAMES_SYSTEM = (
    ...
    "Подбери блюда под запрос: разные, сытные, которые удобно заморозить порциями. "
    "Учитывай ограничения (аллергии, без свинины и т.п.). "
    "Не повторяй недавно принятые блюда. Ответ компактный, на русском. "
    "ВАЖНО: поле name — только короткое название блюда, 2–4 слова, "
    "без описаний, скобок, времени и способа готовки."
)

# Быстрый первый шаг: только названия блюд + мета.
NAMES_SCHEMA = {
    "type": "object",
    "properties": {
        "reply": {"type": "string", "description": "Короткая реплика в чат (1 предложение)"},
        "title": {"type": "string", "description": "Название плана, 2–4 слова"},
        "week_label": {"type": "string", "description": "Например: 14–20 июля"},
        "dishes": {
            "type": "array",
            "items": {
                "type": "object",
    ...
    "required": ["reply", "title", "week_label", "dishes"],
}
...
def build_names_messages(user_message: str, avoid_titles: list[str]) -> list[dict[str, str]]:

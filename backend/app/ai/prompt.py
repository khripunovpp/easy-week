...
FREEZE_RULES = (
    ...
    "сливки, сметана, молоко, йогурт, мягкий сыр при заморозке расслаиваются — вводи их ПОСЛЕ "
    "разморозки/при разогреве; картофель в супах и рагу становится водянистым — не клади в "
    "заморозку, добавляй свежим при подаче (в борщ картофель — отдельно); свежую зелень (укроп, "
    "петрушка, базилик, кинза) добавляй после разморозки; не используй свежие огурцы, листовой "
    "салат, редис, целые сырые помидоры; макароны и рис слегка недоваривай (аль денте); яйца "
    "вкрутую, майонез и желатиновые соусы заморозку переносят плохо; панировка теряет хруст. "
    "Хорошо морозятся: тушёное мясо, котлеты и тефтели, супы без сливок и картофеля, бульоны, "
    "томатные и масляные соусы, рагу, запеканки. "
    "Проблемные компоненты можно оставлять в блюде, но в шагах и note явно писать, что они "
    "добавляются после разморозки/при разогреве. "
)
